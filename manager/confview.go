...
type ConfViewDecl struct {
	AssignTo **ConfView
	Text     declarative.Property
}

func (cv ConfViewDecl) Create(builder *declarative.Builder) error {
	w, err := NewConfView(builder.Parent())
	if err != nil {
		return err
	}
	if cv.AssignTo != nil {
	...
	return builder.InitWidget(cv, w, nil)
}
