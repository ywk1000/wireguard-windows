...
type SyntaxEditDecl struct {
	AssignTo           **SyntaxEdit
	Text               declarative.Property
	OnTextChange       walk.EventHandler
	OnPrivateKeyChange PrivateKeyHandler
}

func (se SyntaxEditDecl) Create(builder *declarative.Builder) error {
	w, err := NewSyntaxEdit(builder.Parent())
	if err != nil {
		return err
	}
	if se.AssignTo != nil {
	...
	return builder.InitWidget(se, w, func() error {
		if se.OnPrivateKeyChange != nil {
			w.PrivateKeyChanged().Attach(se.OnPrivateKeyChange)
		}
		return nil
	})
}
